...
impl DefaultShell {
    pub fn render_with_children(self, children: impl Renderable) -> Rendered<String> {
        ...
        let current_time = {
            let time_str = std::env::var("LUMINITE_LAST_MODIFIED").unwrap_or("sometime".to_string()).parse::<i64>().unwrap_or(0);
            let format_description = time::macros::format_description!( "[year]-[month]-[day] [hour]:[minute]:[second] utc[offset_hour sign:mandatory]");
                        ...
                        div class="flex-1 md:mt-2" {
                            ...
                            main class=("main-content".to_owned() + page_width) {
                                ...
                                footer class="border-t mt-8 border-solid border-muted mb-4 flex justify-between items-center gap-4 text-sm text-muted py-1" {
                                    p class="smallcaps" {"© 2025 Youwen Wu. Generated by "
                                        a 
                                        ...
                                        " using rustc " {(rustc_verison)} " at " {(current_time)} ". Most content cc-by-sa-4.0."
                                    }
                                }
                            }
                        }

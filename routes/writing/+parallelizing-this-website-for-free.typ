...
somewhat slow (in the realm of ~4 seconds total). Since I haven#(apostrophe)t
implemented any sort of hot reload, I need to rebuild the whole site to view
any changes, and it#(apostrophe)s actually a bit cumbersome to do development
...
Here#(apostrophe)s how the build process works at a high level:
...
#smallcaps[html] template in one step. What we#(apostrophe)re going to do
...
pages is already extremely fast, so we#(apostrophe)ll keep it serial for now.
Not to mention, it#(apostrophe)s also prone to race conditions because it
...
#quote[fearless concurrency,] it can#(apostrophe)t guarantee safety from race conditions in
...
Here#(apostrophe)s the refactored code of the original, slow function that does
...
We#(apostrophe)re going to make a one liner change to make it concurrent:
...
And we#(apostrophe)re done! In theory, our tree traversal is now going to run concurrently
thanks to `rayon`.
...
Did it work? It#(apostrophe)s Rust, so of course it worked first try!
...
Here#(apostrophe)s benchmark data using
...
subdirectory. I#(apostrophe)m not exactly sure how `rayon` handles this under
...
I#(apostrophe)m not affected yet.
...
That#(apostrophe)s a problem for later. I#(apostrophe)m quite pleased with how
...
Rustacean technology was more powerful than I could#(apostrophe)ve imagined in
my wildest dreams.

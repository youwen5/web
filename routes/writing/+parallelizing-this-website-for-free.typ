...
#btw[
  ...
  written in Rust, and the pages are generated by Typst.
...
implemented by calling `.iter()` on the `HashMap`, then mapping over the
iterator and invoking recursively on any subtree. This immediately presented an
obvious place to find a lot of performance gains: we should be able to walk
down each subtree in parallel. Querying metadata from a document has nothing to
do with any other document, so it should be perfectly safe to parallelize this.
...
After changing to `.par_iter()` and a few very minimal modifications to my
...
it went from 1-2 seconds to near instant.
...
possible to parallelize the second step, but I#(apostrophe)m afraid of hard to debug IO
